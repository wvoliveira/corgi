...
func main() {
	...
	service = server.LoggingMiddleware(logger)(service)
	service = server.NewMetrics(service)

	// Register APIs endpoint.
	mux.Handle("/api/v1/", server.MakeHTTPHandler(service, httpLogger))
