...
# In default Makefile there is a Dockerfile.in in root of project.
PACKAGE_CONTAINER_FOLDER := ./build/package/container
...
$(foreach bin,$(BINS),$(eval                                         \
    .container-$(subst /,_,$(REGISTRY)/$(bin))-$(TAG): bin/$(OS)_$(ARCH)/$(bin)$(BIN_EXTENSION) $(LICENSES) $(PACKAGE_CONTAINER_FOLDER)/Dockerfile.in  \
))
...
$(CONTAINER_DOTFILES): .buildx-initialized
	...
	sed                                            \
	    ...
	    $(PACKAGE_CONTAINER_FOLDER)/Dockerfile.in > .dockerfile-$(BIN)-$(OS)_$(ARCH)
