...
type User struct {
	...
	Roles     []string  `json:"roles" gorm:"default:user" example:"[user,admin]"`
...
func (s *dbService) PostUser(ctx context.Context, u User) error {
	if u.Name == "" || u.Email == "" {
		return errors.New("fields required: name and email")
	}

	result := s.db.Limit(1).Where("email=?", u.Email).Find(&u)
